...
const char* estimated_time_options = "Процент\nИнтерполяция\nСлайсер";
...
#define PRINTER_SPECIFIC_SETTING global_config.multi_printer_mode ? "Сохр. для принтера" : NULL

void settings_section_theming(lv_obj_t* panel)
{
    lv_obj_t * label = lv_label_create(panel);
    lv_label_set_text(label, "Тема");

    lv_create_custom_menu_dropdown("Цвет темы", panel, theme_dropdown, "Синий\nЗеленый\nЛайм\nСерый\nЖелтый\nОранж.\nКрасный\nФиолет.", get_current_printer()->printer_config->color_scheme, NULL, PRINTER_SPECIFIC_SETTING);
    lv_create_custom_menu_switch("Светлая тема", panel, light_mode_switch, get_current_printer()->printer_config->light_mode, NULL, PRINTER_SPECIFIC_SETTING);
}
...
void settings_section_behaviour(lv_obj_t* panel)
{
    ...
    if (is_klipper)
    {
        lv_create_custom_menu_dropdown("Оценка времени", panel, estimated_time_dropdown, estimated_time_options, get_current_printer()->printer_config->remaining_time_calc_mode, NULL, PRINTER_SPECIFIC_SETTING);
        lv_create_custom_menu_dropdown("Стат. на экр. прогресса", panel, show_stats_on_progress_panel_dropdown, "Ничего\nСлои\nЧастично\nВсе", get_current_printer()->printer_config->show_stats_on_progress_panel, NULL, PRINTER_SPECIFIC_SETTING);
    }
    else if (is_bambu)
    {
        lv_create_custom_menu_dropdown("Стат. на экр. прогресса", panel, show_stats_on_progress_panel_dropdown, "Ничего\nСлои", get_current_printer()->printer_config->show_stats_on_progress_panel, NULL, PRINTER_SPECIFIC_SETTING);
    }
    ...
    lv_create_custom_menu_dropdown("Таймаут экрана", panel, wake_timeout_dropdown, wake_timeout_options, wake_timeout_settings_index);
#endif

#ifndef CYD_SCREEN_DISABLE_TIMEOUT
    lv_create_custom_menu_switch("Экран вкл. при печати", panel, on_during_print_switch, global_config.on_during_print);
#endif

    lv_create_custom_menu_switch("Неск. принтеров", panel, multi_printer_switch, global_config.multi_printer_mode);
    
    if (is_klipper)
    {
        lv_create_custom_menu_switch("Откл. M117", panel, disable_m117_messaging_switch, global_config.disable_m117_messaging);

        lv_create_custom_menu_switch("Свои макросы пластика", panel, filament_move_mode_switch, get_current_printer()->printer_config->custom_filament_move_macros, NULL, 
            global_config.multi_printer_mode
                ? "Вызывает FILAMENT_RETRACT и\nFILAMENT_EXTRUDE в меню темп.\nСохр. для принтера."
                : "Вызывает FILAMENT_RETRACT и\nFILAMENT_EXTRUDE в меню темп.");

        lv_create_custom_menu_switch("Показ. Экстр. Стоп", panel, show_estop_switch, global_config.show_estop);
    }

    lv_create_custom_menu_switch("Сорт. макросов A->Z", panel, sort_macros_switch, global_config.sort_macros);
    lv_create_custom_menu_switch("Полные имена файлов", panel, full_filenames_switch, global_config.full_filenames);
    lv_create_custom_menu_switch("Увелич. карт. Gcode", panel, double_size_gcode_img_switch, global_config.double_size_gcode_img);
    lv_create_custom_menu_button("Настройка хоста", panel, reset_ip_click, "Рестарт");
}
...
void settings_section_device(lv_obj_t* panel)
{
    ...
    lv_create_custom_menu_switch("Инверсия цветов", panel, invert_color_switch, global_config.printer_config[global_config.printer_index].invert_colors, NULL, (global_config.multi_printer_mode) ? "Сохр. для принтера" 
    #ifdef CYD_SCREEN_DRIVER_ESP32_2432S028R
        "\nТолько для 2.8\" dual USB" :  "Только для 2.8\" dual USB"
    #else
       : NULL
    #endif
    );
...
#ifdef CYD_SCREEN_DRIVER_ESP32_2432S028R
    lv_create_custom_menu_switch("Фикс цветов экрана", panel, dualusb_screen_fix_switch, global_config.display_mode, NULL, "Только для 2.8\" dual USB");
#endif
...
#else
    lv_create_custom_menu_switch("Поворот экрана", panel, rotate_screen_switch, global_config.rotate_screen);
#endif

    lv_create_custom_menu_switch("Авто-обновление", panel, auto_ota_update_switch, global_config.auto_ota_update);
    lv_create_custom_menu_label("Версия", panel, REPO_VERSION "  ");
    ...
    lv_create_custom_menu_button("Калибровка тача", panel, reset_calibration_click, "Рестарт");
#endif // CYD_SCREEN_DISABLE_TOUCH_CALIBRATION

    lv_create_custom_menu_button("Настройка WiFi", panel, reset_wifi_click, "Рестарт");
    lv_create_custom_menu_button("Перезагрузка", panel, reset_click, "Рестарт");
}
